...
{
    public class ArchModelBinder : DefaultModelBinder
    {
        /// <summary>
        ...
        private bool IsEntityType(Type propertyType)
        {
            ...
            if (baseType != null)
            {
                if (baseType.IsGenericType && (baseType.GetGenericTypeDefinition() == typeof(DomainObject<,>) || baseType.GetGenericTypeDefinition() == typeof(LookupObject<,>)))
                {
                    isEntityType = true;
                }
            }
        ...
        public override object ConvertTo(Type type, CultureInfo culture)
        {
            ...
            Type entityInterfaceType = collectionEntityType.GetInterfaces()
                .First(interfaceType => interfaceType.IsGenericType
                    && interfaceType.GetGenericTypeDefinition() == typeof(DomainObject<,>));
            */

            Type entityInterfaceType = collectionEntityType.BaseType;

            Type idType = entityInterfaceType.GetGenericArguments().Skip(1).Take(1).Single();
    ...
    internal class EntityValueProviderResult : ValueProviderResult
    {
        ...
        public override object ConvertTo(Type type, CultureInfo culture)
        {
            ...
            Type entityInterfaceType = propertyType.BaseType;
        ...
        private object GetEntityFor(object typedId, Type idType)
        {
            ...
            //We are going to use GetBytID here which uses Load so it doesn't have to hit the DB
            return entityRepository.GetType()
              .InvokeMember("GetByID", BindingFlags.InvokeMethod, null, entityRepository, new[] { typedId });
        }
    ...
    internal class GenericRepositoryFactory
    {
        public static object CreateEntityRepositoryFor(Type entityType, Type idType)
        {
            ...
            Type concreteRepositoryType =
                genericRepositoryType.MakeGenericType(new Type[] { entityType });

            object repository;

            try
            {
                repository = Activator.CreateInstance(concreteRepositoryType); //ServiceLocator.Current.GetService(concreteRepositoryType);
            }
